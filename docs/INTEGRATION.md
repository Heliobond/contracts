...
```typescript
const proposalId = scValToNative(
  await invokeContract(
    REGISTRY,
    "create_proposal",
    [
...
// Step 2: deposit
const sharesMinted = scValToNative(
  await invokeContract(
    VAULT,
    "deposit",
    [
...
```typescript
const usdcReturned = scValToNative(
  await invokeContract(
    VAULT,
    "withdraw",
    [
